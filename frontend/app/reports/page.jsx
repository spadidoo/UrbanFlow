...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://backend.urbanflowph.com'}/api/reports/list?user_id=${userId}`
      );
  ...
  const handleExport = async (reportId, format) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://backend.urbanflowph.com'}/api/reports/${reportId}/export?format=${format}`,
        {
          method: 'GET',
        }
      );
